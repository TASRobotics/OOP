...
boolean polygonCircleIntersect(ArrayList<PVector> polygon, PVector center, float radius) {
  // 1. Check each edge of the polygon
  for (int i = 0; i < polygon.size(); i++) {
    PVector A = polygon.get(i);
    PVector B = polygon.get((i + 1) % polygon.size()); // wrap around
    if (segmentCircleIntersect(A, B, center, radius)) {
      return true;
    }
  }
...
// Check if a line segment AB intersects the circle
boolean segmentCircleIntersect(PVector A, PVector B, PVector C, float r) {
  PVector AB = PVector.sub(B, A);
  PVector AC = PVector.sub(C, A);
  float ab2 = AB.magSq();
  float t = PVector.dot(AC, AB) / ab2;     // projection factor
  t = constrain(t, 0, 1);                  // clamp t to [0,1]
  PVector closest = PVector.add(A, PVector.mult(AB, t));
  float dist = PVector.dist(closest, C);
  return dist <= r;
}
